...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::PageUp:
		displayOctants = (!displayOctants);
		break;
	case sf::Keyboard::PageDown:
		displayOctants = (!displayOctants);
		break;
	case sf::Keyboard::Add:
		if (m_uOctantLevels < 7)
		{
			m_pEntityMngr->ClearDimensionSetAll();
			++m_uOctantLevels;
			SafeDelete(m_pRoot);
			m_pRoot = new Octant2();
			m_pRoot->SetMaxLevels(m_uOctantLevels);
			m_pRoot->CalcTree();
		}
		break;
...
#pragma region Camera Position
	...
	else
	{
		...
		if (pressed)
		{
			...
			/*quaternion horzQuat = glm::angleAxis(glm::radians(rotationX), AXIS_Y);
			newPos = newPos * horzQuat;
			quaternion vertQuat = glm::angleAxis(glm::radians(rotationY), AXIS_X);
			newPos = newPos * vertQuat;*/

			quaternion test = glm::eulerAngleXY(glm::radians(rotationX), glm::radians(rotationY));
			newPos = newPos * test;
